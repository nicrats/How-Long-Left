...
class MagdaleneBreaks {
    ...
    func getBreaks(events: [HLLEvent], overrideDefaults: Bool = false) -> [HLLEvent] {
        ...
        for event in events {
            ...
            if event.endDate.timeIntervalSinceNow > negativeValue, event.startDate.timeIntervalSinceNow < TimeInterval.week*1.5 {
        ...
        for (index, event) in checkEvents.enumerated() {
            ...
            if var recessEvent = getRecessForDate(event.startDate), let endTimes = endTimesDictionary[startMidnight], let startTimes = startTimesDictionary[startMidnight], !addedRecessDates.contains(startMidnight) {
                
                
                if checkEvents.indices.contains(index-1) {
                    recessEvent.associatedCalendar = checkEvents[index-1].calendar
                }
            ...
            if var lunchEvent = getLunchForDate(event.startDate), let endTimes = endTimesDictionary[startMidnight], let startTimes = startTimesDictionary[startMidnight], !addedLunchDates.contains(startMidnight) {
